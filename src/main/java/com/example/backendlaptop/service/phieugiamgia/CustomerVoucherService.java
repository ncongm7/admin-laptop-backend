...
public class CustomerVoucherService {
    ...
    private String validateVoucher(PhieuGiamGia voucher, UUID khachHangId) {
        ...
        // Kiểm tra lịch sử sử dụng của khách hàng (nếu đã đăng nhập)
        if (khachHangId != null) {
            boolean daSuDung = hoaDonRepository.existsByIdKhachHang_IdAndIdPhieuGiamGia_IdAndTrangThaiNot(
                    khachHangId, voucher.getId(), com.example.backendlaptop.model.TrangThaiHoaDon.DA_HUY);
            if (daSuDung) {
    ...
    public List<VoucherSuggestionResponse> getVoucherSuggestions(UUID khachHangId, BigDecimal tongTienGioHang) {
        ...
        List<VoucherSuggestionResponse> suggestions = phieuGiamGias.stream()
                ...
                .filter(pgg -> {
                    // Filter additional check: usage history for customer
                    if (khachHangId != null) {
                        boolean daSuDung = hoaDonRepository.existsByIdKhachHang_IdAndIdPhieuGiamGia_IdAndTrangThaiNot(
                                khachHangId, pgg.getId(), com.example.backendlaptop.model.TrangThaiHoaDon.DA_HUY);
                        if (daSuDung) {
