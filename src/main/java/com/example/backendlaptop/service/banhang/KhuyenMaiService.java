        ...
        List<VoucherSuggestionResponse> suggestions = phieuGiamGias.stream()
                .filter(pgg -> {
                    ...
                    if (pgg.getTrangThai() == null || pgg.getTrangThai() != 1) {
                        System.out.println(
                                "    ❌ Bị loại: Trạng thái không hoạt động (trangThai=" + pgg.getTrangThai() + ")");
                        return false;
                    }
                    ...
                    if (pgg.getNgayBatDau() != null && pgg.getNgayBatDau().isAfter(now)) {
                        System.out.println(
                                "    ❌ Bị loại: Chưa đến ngày bắt đầu (ngayBatDau=" + pgg.getNgayBatDau() + ")");
                        return false; // Chưa đến ngày bắt đầu
                    }
                    if (pgg.getNgayKetThuc() != null && pgg.getNgayKetThuc().isBefore(now)) {
                        System.out.println("    ❌ Bị loại: Đã hết hạn (ngayKetThuc=" + pgg.getNgayKetThuc() + ", now="
                                + now + ")");
                    ...
                    if (pgg.getSoLuongDung() != null && pgg.getSoLuongDung() <= 0) {
                        System.out
                                .println("    ❌ Bị loại: Hết lượt sử dụng (soLuongDung=" + pgg.getSoLuongDung() + ")");
                    ...
                    if (pgg.getHoaDonToiThieu() != null && tongTien.compareTo(pgg.getHoaDonToiThieu()) < 0) {
                        System.out.println("    ❌ Bị loại: Tổng tiền chưa đủ (tongTien=" + tongTien
    ...
    public HoaDonResponse apDungVoucher(UUID idHoaDon, ApDungVoucherRequest request) {
        ...
        PhieuGiamGia phieuGiamGia = phieuGiamGiaRepository.findById(request.getIdPhieuGiamGia())
                .orElseThrow(() -> new ApiException(
                        "Không tìm thấy phiếu giảm giá với ID: " + request.getIdPhieuGiamGia(), "NOT_FOUND"));
        ...
        // Chỉ giảm số lượng với voucher công khai (không riêng tư)
    ...
    public HoaDonResponse xoaVoucher(UUID idHoaDon) {
        ...
        if (hoaDon.getTrangThai() != null && hoaDon.getTrangThai() != TrangThaiHoaDon.CHO_THANH_TOAN) {
            throw new ApiException(
                    "Chỉ có thể xóa voucher khỏi hóa đơn đang chờ thanh toán. Trạng thái: " + hoaDon.getTrangThai(),
                    "INVALID_STATUS");
        }

        // 3. Hoàn trả số lượng voucher (nếu có và không phải voucher riêng tư)
        if (hoaDon.getIdPhieuGiamGia() != null) {
            PhieuGiamGia pgg = hoaDon.getIdPhieuGiamGia();
            if (!Boolean.TRUE.equals(pgg.getRiengTu()) && pgg.getSoLuongDung() != null) {
