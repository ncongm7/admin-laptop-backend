...
public class ChatController {
    ...
    /**
     * Manual escalation: Customer requests human support
     */
    @PostMapping("/conversation/{conversationId}/escalate-to-human")
    public ResponseEntity<ResponseObject<Void>> escalateToHuman(@PathVariable UUID conversationId) {
        log.info("🚨 [API] Manual escalation to human for conversation: {}", conversationId);
        chatbotService.escalateConversation(conversationId);
        return ResponseEntity.ok(new ResponseObject<>(null, "Đã chuyển sang chế độ hỗ trợ nhân viên"));
    }

    /**
     * Return to bot: Customer wants to continue with AI
     */
    @PostMapping("/conversation/{conversationId}/return-to-bot")
    public ResponseEntity<ResponseObject<Void>> returnToBot(@PathVariable UUID conversationId) {
        log.info("🤖 [API] Return to bot for conversation: {}", conversationId);
        chatService.returnToBot(conversationId);
        return ResponseEntity.ok(new ResponseObject<>(null, "Đã quay lại chế độ tư vấn AI"));
    }

    /**
     * Get escalation status
     */
    @GetMapping("/conversation/{conversationId}/escalation-status")
    public ResponseEntity<ResponseObject<java.util.Map<String, Object>>> getEscalationStatus(
            @PathVariable UUID conversationId) {
        var status = chatService.getEscalationStatus(conversationId);
        return ResponseEntity.ok(new ResponseObject<>(status, "Lấy trạng thái escalation thành công"));
    }
}
