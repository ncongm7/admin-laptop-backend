...
public interface SerialRepository extends JpaRepository<Serial, UUID> {
    ...
    // Dieu kien: TrangThai=1 (chua ban thuc su), Reserved boi Don Online (Loai=1), Chua thanh toan (TTTT=0)
    // Tìm serial đang bị giữ bởi đơn Online COD (có thể "cướp" nếu là POS Cash)
    ...
    List<com.example.backendlaptop.entity.HoaDon> findOrdersWithExpiredReservations(@Param("now") java.time.Instant now);

    // Tìm serial đã mua của khách hàng (theo KhachHang ID)
    // Query thông qua HoaDonChiTiet: Tìm các CTSP đã được mua trong đơn hàng của khách hàng
    // Sau đó tìm các serial thuộc CTSP đó và đã được bán (trangThai = 2)
    @Query("SELECT s FROM Serial s WHERE s.ctsp.id IN " +
           "(SELECT hdct.chiTietSanPham.id FROM HoaDonChiTiet hdct " +
           "WHERE hdct.hoaDon.idKhachHang.id = :userId " +
           "AND hdct.hoaDon.trangThai >= 2) " +
           "AND s.trangThai = 2")
