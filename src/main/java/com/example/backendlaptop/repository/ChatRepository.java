...
public interface ChatRepository extends JpaRepository<Chat, UUID> {
    ...
    @Query("SELECT COUNT(c) > 0 FROM Chat c WHERE c.conversationId = :conversationId AND c.isFromCustomer = false AND c.isBotMessage = false")
    boolean hasStaffJoined(@Param("conversationId") UUID conversationId);
}
